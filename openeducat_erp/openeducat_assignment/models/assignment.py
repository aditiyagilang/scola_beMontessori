...
class OpAssignment(models.Model):
    ...
    def write(self, vals):
        res = super(OpAssignment, self).write(vals)
        # Tambahkan siswa ke allocation_ids jika batch_id diubah
        if 'batch_id' in vals:
            students = self.env['op.student'].search([('batch_id', '=', self.batch_id.id)])
            self.allocation_ids = [(6, 0, students.ids)]
        return res

    @api.model_create_multi
    def create(self, vals_list):
        assignments = super(OpAssignment, self).create(vals_list)
        for assignment in assignments:
            # Tambahkan siswa ke allocation_ids berdasarkan batch_id
            students = self.env['op.student'].search([('batch_id', '=', assignment.batch_id.id)])
            assignment.allocation_ids = [(6, 0, students.ids)]
        return assignments
