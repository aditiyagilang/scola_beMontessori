...
class OpSession(models.Model):
    ...
    session_time_display = fields.Char(string="Session Time", compute="_compute_session_time", store=True)
    ...
    def _compute_timing(self):
        for session in self:
            session.timing = f"{session.start_datetime.strftime('%I:%M%p')} - {session.end_datetime.strftime('%I:%M%p')}"

    @api.depends('start_datetime', 'end_datetime')
    def _compute_session_time(self):
        for record in self:
            if record.start_datetime and record.end_datetime:
                record.session_time_display = f"{record.start_datetime.strftime('%I:%M %p')} - {record.end_datetime.strftime('%I:%M %p')}"

    @api.onchange('lesson_schedule_id')
    def _onchange_lesson_schedule(self):
        if self.lesson_schedule_id and self.start_datetime:
            current_date = self.start_datetime.date()

            try:

                if isinstance(self.lesson_schedule_id.start_time, str):
                    hour_start, minute_start = map(int, self.lesson_schedule_id.start_time.split(':'))
                    hour_end, minute_end = map(int, self.lesson_schedule_id.end_time.split(':'))
                else:
                    hour_start = int(self.lesson_schedule_id.start_time)
                    minute_start = int((self.lesson_schedule_id.start_time - hour_start) * 60)
                    hour_end = int(self.lesson_schedule_id.end_time)
                    minute_end = int((self.lesson_schedule_id.end_time - hour_end) * 60)

                start_time = datetime.combine(current_date, time(hour_start, minute_start)) - timedelta(hours=7)
                end_time = datetime.combine(current_date, time(hour_end, minute_end)) - timedelta(hours=7)

                self.start_datetime = start_time
                self.end_datetime = end_time

            except ValueError:
                raise ValueError("Format waktu pada Lesson Schedule harus HH:MM atau float (ex: 8.5 untuk 08:30).")
