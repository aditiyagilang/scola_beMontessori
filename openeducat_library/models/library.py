...
class OpLibraryCard(models.Model):
   ...
   #def create(self, vals_list):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            # Tambahkan validasi untuk field mandatory
            if "type" not in vals:
                raise ValidationError("Field 'type' wajib dsdisi.")
        ...
        # Pastikan res tidak None sebelum mengakses res.type
        if res and res.type == "student":
            res.student_id.library_card_id = res
        elif res:
            res.faculty_id.library_card_id = res
        return res
